...
function generateMockPhotos(): Photo[] {
  ...
  // Warm earth tones -- golden hour, sienna, ochre, sage
  const hues = [
    18, 28, 38, 185, 205, 155, 345, 32, 48, 195, 168, 22,
    30, 192, 172, 40, 14, 208, 24, 180, 34, 212, 162, 42,
    20, 188, 175, 44,
  ]
  ...
  for (let i = 0; i < 28; i++) {
    ...
    photos.push({
      ...
      hue: hues[i],
      saturation: 28 + Math.floor(seededRandom(i + 20) * 30),
      lightness: 20 + Math.floor(seededRandom(i + 40) * 20),
    })
  }
  return photos
}
...
function StatChip({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group flex items-center gap-2 rounded-full px-3.5 py-1.5 text-xs font-medium transition-all duration-150',
        active
...
// ---------------------------------------------------------------------------
// Photo card -- grid view (editorial exhibition style)
// ---------------------------------------------------------------------------
...
function PhotoCard({
  ...
  const bgColorLight = `hsl(${photo.hue}, ${photo.saturation + 8}%, ${photo.lightness + 6}%)`
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${bgColor} 0%, ${bgColorLight} 55%, ${bgColor} 100%)`,
        }}
      >
        {/* Subtle light flare overlay */}
        <div className="absolute inset-0 opacity-[0.04]" style={{
          backgroundImage: 'radial-gradient(circle at 25% 35%, rgba(255,255,255,0.9) 0%, transparent 45%), radial-gradient(circle at 75% 65%, rgba(255,255,255,0.4) 0%, transparent 35%)',
        }} />

        {/* Film frame number */}
        <div
          className="absolute bottom-1.5 right-2.5 text-[9px] tracking-widest text-white/15"
          style={{ fontFamily: '"Fira Code", ui-monospace, monospace' }}
        >
      ...
      <div className={cn(
        ...
        <div className="p-3.5">
          <p
            className="truncate text-[13px] font-medium text-white/95"
            style={{ fontFamily: '"Fira Code", ui-monospace, monospace' }}
          >
            {photo.filename}
          </p>
          <div className="mt-1 flex items-center gap-2">
            ...
            <span className="text-[11px] text-white/55 tracking-wide">{STATUS_LABEL[photo.status]}</span>
        ...
        {!batchMode && (
          <div className="absolute right-3 top-3 rounded-full bg-black/35 p-2 backdrop-blur-sm">
      ...
      {/* Status dot on card */}
      {!batchMode && (
        <div className="absolute bottom-2.5 left-3 group-hover:opacity-0 transition-opacity duration-200">
...
function PhotoRow({
  ...
  return (
    <button
      ...
      onClick={batchMode ? onSelect : onClick}
      className={cn(
        'group flex w-full items-center gap-4 rounded-2xl px-4 py-2.5 text-left transition-all duration-150',
        isSelected && batchMode
    ...
    >
      ...
      <div className="flex items-center gap-1.5 w-24 shrink-0">
        <span className={cn('h-1.5 w-1.5 rounded-full', STATUS_DOT[photo.status])} />
        <span className={cn('text-xs tracking-wide', STATUS_TEXT[photo.status])}>
...
// ---------------------------------------------------------------------------
// Detail Panel -- editorial photo viewer
// ---------------------------------------------------------------------------
...
function DetailPanel({
  ...
  return (
    <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" style={{ fontFamily: '"Outfit", system-ui, sans-serif' }}>
      {/* Backdrop */}
      <div
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center gap-0">
        ...
        {/* Main preview area */}
        <div className="flex flex-1 flex-col items-center justify-center px-14 py-10">
          <div className="relative flex w-full max-w-3xl items-center justify-center">
            ...
            {/* Image */}
            <div
              className="aspect-[3/2] w-full max-w-3xl overflow-hidden rounded-2xl shadow-2xl shadow-black/40"
              style={{
        ...
        {/* Metadata sidebar */}
        <div className="relative h-full w-[360px] shrink-0 border-l border-border bg-card">
          <div className="flex h-full flex-col overflow-y-auto">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-border px-6 py-5">
              <h3 className="text-base font-semibold tracking-tight text-foreground">Details</h3>
              <button
                type="button"
                onClick={onClose}
                ...
                {metaRows.map((row) => (
                  <div key={row.label} className="flex items-start gap-3.5">
                    ...
                    <div className="min-w-0 flex-1 pt-0.5">
                      ...
                      )} style={row.mono ? { fontFamily: '"Fira Code", ui-monospace, monospace' } : undefined}>
                        {row.label === 'Status' ? (
            ...
            <div className="border-t border-border px-6 py-5 space-y-2.5">
              <button
                type="button"
                className={cn(
                  'flex w-full items-center justify-center gap-2.5 rounded-2xl px-4 py-3',
...
export default function GalleryB2(): React.JSX.Element {
  ...
  return (
    <div className="relative flex h-full flex-col bg-background" style={{ fontFamily: '"Outfit", system-ui, sans-serif' }}>
        ...
        <div className="flex items-center gap-5 px-8 pt-6 pb-4">
          ...
          <div className="relative w-72">
            ...
            <input
              ...
              className={cn(
                'h-9 w-full rounded-full border border-border bg-secondary/50 pl-10 pr-9 text-[13px] text-foreground',
          ...
          <div className="relative" ref={sortRef}>
            <button
              ...
              className={cn(
                'flex h-9 items-center gap-2 rounded-full border border-border px-3.5 text-xs text-muted-foreground transition-all duration-150',
                'hover:text-foreground hover:bg-secondary/50',
                showSortMenu && 'border-foreground/20 text-foreground bg-secondary/50',
              )}
            >
              <SlidersHorizontal className="h-3.5 w-3.5" />
              <span className="hidden sm:inline tracking-wide">{sortOptions.find((s) => s.mode === sortMode)?.label}</span>
            </button>
            {showSortMenu && (
              <div className="absolute right-0 top-full z-30 mt-2 w-48 rounded-2xl border border-border bg-card py-1.5 shadow-xl shadow-black/20">
                {sortOptions.map((opt) => (
                  <button
                    key={opt.mode}
                    type="button"
          ...
          <button
            ...
            className={cn(
              'flex h-9 items-center gap-2 rounded-full border px-4 text-xs font-medium transition-all duration-150',
              batchMode
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredPhotos.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-28 text-center">
            <div className="mb-5 flex h-18 w-18 items-center justify-center rounded-2xl bg-secondary">
            ...
            <button
              ...
              className="mt-5 rounded-full bg-secondary px-5 py-2.5 text-sm font-medium text-foreground transition-colors hover:bg-accent"
            >
              Reset Filters
            </button>
          </div>
        ) : viewMode === 'grid' ? (
          /* Grid view -- editorial gallery */
          ...
          <div className="px-8 py-4">
            {/* List header */}
            <div className="mb-1 flex items-center gap-4 px-4 py-2 text-[11px] font-medium uppercase tracking-[0.1em] text-muted-foreground">
      ...
      {batchMode && selectedPhotos.size > 0 && (
        <div className="absolute bottom-6 left-1/2 z-30 -translate-x-1/2 animate-in slide-in-from-bottom-4 fade-in duration-200">
          <div className={cn(
            'flex items-center gap-2.5 rounded-2xl border border-border bg-card px-5 py-3',
            ...
            <button
              type="button"
              className={cn(
                'inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium transition-all duration-150',
                'bg-secondary text-foreground',
