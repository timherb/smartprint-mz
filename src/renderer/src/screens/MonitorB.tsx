...
// ---------------------------------------------------------------------------
// Animated counter hook — numbers tick up on mount
// ---------------------------------------------------------------------------
    ...
    <div className="flex items-center gap-2.5">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className={cn('text-[11px] font-mono font-medium', isLow ? 'text-amber-accent' : 'text-foreground')}>
            {value}%
          </span>
        </div>
...
function HeroStat({
  ...
  return (
    ...
    >
      {/* Subtle amber glow behind the number for the primary stat */}
      {glow && (
        <div className="absolute -top-8 -right-8 h-28 w-28 rounded-full bg-amber-accent/[0.06] blur-2xl transition-opacity duration-500 group-hover:opacity-100" />
      )}
      ...
      <div className="mt-3 flex items-baseline gap-1 relative">
        ...
        {suffix && (
          <span className={cn(
            ...
            glow ? 'text-amber-accent/70' : 'text-muted-foreground'
          )}>
            {suffix}
          </span>
        )}
...
// ---------------------------------------------------------------------------
// Job status badge with contextual styling
// ---------------------------------------------------------------------------
...
function JobBadge({ status }: { status: JobStatus }): React.JSX.Element {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[11px] font-medium', cfg.classes)}>
...
function PrinterCard({ printer }: { printer: PrinterInfo }): React.JSX.Element {
  ...
  return (
    <div className={cn(
      ...
      'hover:border-border/80',
      printer.status === 'warning' && 'border-amber-accent/15',
    )}>
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <div className="min-w-0">
          <p className="text-sm font-semibold text-foreground truncate">{printer.name}</p>
          <p className="text-xs text-muted-foreground font-mono">{printer.model}</p>
        </div>
      ...
      <div className="flex items-baseline gap-1 mb-4">
        ...
        <span className="text-xs text-muted-foreground ml-1">prints</span>
      ...
      {/* Current job */}
      {printer.currentJob ? (
        <div className="flex items-center gap-2 rounded-lg bg-glow px-3 py-2 mb-4">
          <Loader2 className="h-3.5 w-3.5 text-amber-accent animate-spin" />
          <span className="text-xs font-medium text-amber-accent font-mono truncate">
            {printer.currentJob}
          </span>
        </div>
      ) : (
        <div className="flex items-center gap-2 rounded-lg bg-secondary px-3 py-2 mb-4">
  ...
  return (
    <div className="flex gap-3 group">
      ...
      <div className={cn('pb-4 min-w-0', isLast && 'pb-0')}>
        ...
        <p className="mt-0.5 text-[11px] font-mono text-muted-foreground">{event.timestamp}</p>
      </div>
    </div>
  )
}
...
function QueueRow({ job }: { job: PrintJob }): React.JSX.Element {
  ...
  return (
    <div
      className={cn(
        'group flex items-center gap-4 rounded-xl border bg-card px-4 py-3.5 transition-all duration-200',
        isPrinting && 'border-amber-accent/20 bg-glow/50',
        isError && 'border-red-500/20',
        isComplete && 'border-border opacity-60 hover:opacity-100',
    ...
    >
      ...
      <div className={cn(
        ...
        <FileImage className={cn('h-4.5 w-4.5', isPrinting ? 'text-amber-accent' : 'text-muted-foreground')} />
      </div>

      {/* Info column */}
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2.5">
          <span className="text-sm font-medium text-foreground truncate font-mono">{job.filename}</span>
          <JobBadge status={job.status} />
        </div>
        <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-muted-foreground">
          {!isQueued && <span className="font-mono">{job.printer}</span>}
          ...
          {job.elapsed && (
            <>
              <span>&middot;</span>
              <span className="font-mono">{job.elapsed}</span>
            </>
          )}
        </div>

        {/* Progress bar for printing/error */}
        {(isPrinting || isError) && (
      ...
      <div className="shrink-0 w-16 text-right">
        {isPrinting && (
          <span className="text-sm font-bold font-mono text-amber-accent">{job.progress}%</span>
        )}
        {isComplete && (
          <CheckCircle2 className="h-4.5 w-4.5 text-emerald-500 ml-auto" />
        )}
        {isError && (
          <button
            type="button"
            className="inline-flex items-center gap-1 rounded-lg bg-red-500/10 px-2.5 py-1 text-[11px] font-medium text-red-400 transition-colors hover:bg-red-500/20"
          >
        ...
        {isQueued && (
          <button
            type="button"
            className="inline-flex items-center gap-1 rounded-lg bg-secondary px-2.5 py-1 text-[11px] font-medium text-muted-foreground transition-colors hover:bg-secondary/80 hover:text-foreground opacity-0 group-hover:opacity-100"
          >
...
export default function MonitorB(): React.JSX.Element {
  const [isConnected] = useState(true)
  const [isPaused, setIsPaused] = useState(false)
  const [now, setNow] = useState(new Date())

  // Tick the clock every second to feel alive
  useEffect(() => {
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      {!isConnected && (
        <div className="flex items-center justify-center gap-2 bg-red-500/10 border-b border-red-500/20 px-4 py-2.5 text-sm text-red-400">
          ...
          <button
            type="button"
            className="ml-3 rounded-md bg-red-500/15 px-3 py-1 text-xs font-semibold transition-colors hover:bg-red-500/25"
          >
            Retry Now
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-[1280px] px-8 py-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div>
                <div className="flex items-center gap-2.5">
                  <h2 className="text-lg font-bold text-foreground tracking-tight">Live Monitor</h2>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2">
                ...
                <span className="text-xs font-mono font-medium text-foreground tabular-nums">{clockStr}</span>
              </div>
              ...
              <button
                ...
                className={cn(
                  'inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-xs font-semibold transition-all duration-200',
                  isPaused
          ...
          <div className="grid grid-cols-[1fr_320px] gap-6">
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[11px] text-muted-foreground font-mono">Latest</span>
              </div>
